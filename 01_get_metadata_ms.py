...
# > activate micasense
# > python  C:/script/python/micasense/ip/geo/01_get_metadata_ms.py
###############################################################################
...
image_path = filedialog.askdirectory(parent=root,
    initialdir=r'C:/Users/Usuario/Desktop/micasense/imagenes/set',
...
try:
    ...
    # Escribir comando CMD
        batWrite.write("{0}:\ncd {1}\n{2} -csv -r IMG_*.tif -GPSLatitude -GPSLongitude -IrradianceYaw > {3}/00_GNSS_DecSecMin.txt".format(image_path[:1],image_path, exiftoolPath, outs_dir))
